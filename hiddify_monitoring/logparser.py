...
def parse(line):
    ...
    if data:
        ...
        datadic['dateandtime'] = datetime.strptime(
            datadic['dateandtime'], "%d/%b/%Y:%H:%M:%S %z")
        date = datadic['dateandtime'].strftime('%Y%m%d')
        datadic['upstream'] = upstream_map.get(
            datadic['upstream'], datadic['upstream'])
        ...
        datadic['haship'] = haship(
            f"{datadic['ipinfo']['asn_name']}{date}{haship(datadic['ipaddress'])}")
